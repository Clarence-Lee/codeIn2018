...
public class toutiao01{
    ...
    public toutiao01(char[][] map,int r,int c,int begx,int begy,int endx,int endy,int begsx,int begsy){
        this.map=map;
        this.r=r;
        this.c=c;
        this.begx=begx;
    ...
    public static void main(String args[]) {
        ...
        while(in.hasNext()){
            ...
            for(int i = 0; i <r; i++){
                ...
                for(int j=0;j< c;j++){
                    map[i][j]=s.charAt(j);
                    if (map[i][j] == '0') { begx = i; begy = j;}//箱子开始坐标
                    if (map[i][j] == 'E') { endx = i; endy = j;}//目标坐标
                    if (map[i][j] == 'S')  { begsx = i;begsy = j;}//人开始坐标
                }

            }
            toutiao01 ma=new toutiao01(map,r,c,begx,begy,endx,endy,begsx,begsy);
